...
- **Installer is distro-aware** (#76). The bundled `install.sh`
  generated by `scripts/package-linux.sh` now reads `/etc/os-release`
  and adapts its install/uninstall hints to the host's package
  manager. The "Docker not installed" error suggests
  `apt-get install`, `dnf install`, `zypper install`, or `pacman -Sy`
  depending on the distro family (debian/rhel/suse/arch). The
  CasaOS-coexistence remove suggestion no longer hardcodes
  `apt remove` — it picks the correct command for the host. PowerLab
  binaries themselves were already distro-agnostic; this PR removes
  the only Debian-only assumptions left in the installer.
