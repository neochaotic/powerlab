...
| Surface | When | Notes |
|---|---|---|
| Maintainer sync | When curating | Maintainers run `make sync-catalog` (writes to repo) or `make sync-catalog-dry` to bulk-import + filter upstream apps, then hand-curate the survivors |
| Install / upgrade | Every install | `install.sh` wipes `community-catalog/Apps/` and re-bundles the tarball's curated set. **No automatic upstream sync** (ADR-0039). |
| Manual operator sync | Any time | `/usr/bin/powerlab-sync-catalog` ships in `/usr/bin` for explicit operator use against a custom upstream. Output should go to a dedicated path and be registered as a custom catalog source via Settings → Catalog. |

### Tarball + binary architecture (v0.7.1+)

Per ADR-0039, the curated catalog is fully owned and vendored: the tarball's `community-catalog/Apps/` IS the catalog. `install.sh` removes `/var/lib/powerlab/community-catalog/Apps/` and replaces it with the tarball tree — wipe-then-copy, not overlay. This guarantees the installed state matches the release byte-for-byte and closes the orphan-survives-upgrade bug class (#450).

- `scripts/package-linux.sh` cross-compiles `backend/sync-catalog` alongside the 6 backend services and ships it at `/usr/bin/powerlab-sync-catalog` (~9 MB Go binary)
- The binary is NOT invoked by `install.sh`; ADR-0039 forbids implicit upstream syncing
- Operators who want to mirror a custom upstream run the binary by hand and register the output as a custom catalog source via Settings → Catalog (Unaudited badge)
- Maintainers use the binary in CI / Makefile to bulk-import upstream apps when curating new additions to `community-catalog/`

Regression test at `scripts/check-package-linux-adr-0039_test.sh` locks the wiring: wipe-before-copy must be present, post-install auto-invocation must NOT.
