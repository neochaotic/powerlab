...
func (a *AppManagement) InstallComposeApp(ctx echo.Context, params codegen.InstallComposeAppParams) error {
	...
	// Port conflicts are checked in two cases only:
	//  1) dry_run — the caller wants to know in advance (e.g. Custom App Builder)
	//  2) explicit CheckPortConflict=true — the legacy strict-mode call
	// On a normal install we let `compose_service.Install` auto-remap conflicting
	// ports to the next available one. This prevents the install from failing
	// when a port like 8080 is already taken.
	wantsStrict := params.CheckPortConflict != nil && *params.CheckPortConflict
	wantsDryRun := params.DryRun != nil && *params.DryRun
	if wantsStrict || wantsDryRun {
...
func (a *AppManagement) ComposeAppStats(ctx echo.Context, id codegen.ComposeAppID) error {
	if id == "" {
		message := ErrComposeAppIDNotProvided.Error()
		return ctx.JSON(http.StatusBadRequest, codegen.ResponseBadRequest{
			Message: &message,
	...
	stats, err := composeApp.Stats(ctx.Request().Context())
	if err != nil {
		message := err.Error()
		return ctx.JSON(http.StatusInternalServerError, codegen.ResponseInternalServerError{Message: &message})
	}

	return ctx.JSON(http.StatusOK, codegen.Response{
		Data: stats,
	})
}

// ComposeAppDiskUsage returns the on-disk size of an installed app's data
// directory. Uses the same `compose.List()` lookup pattern as the other
// /compose/{id}/* endpoints to remain consistent and surface the same 404
// behaviour for not-yet-installed apps.
func (a *AppManagement) ComposeAppDiskUsage(ctx echo.Context, id codegen.ComposeAppID) error {
	if id == "" {
		message := ErrComposeAppIDNotProvided.Error()
		return ctx.JSON(http.StatusBadRequest, codegen.ResponseBadRequest{Message: &message})
	}
	...
	bytes, err := composeApp.DiskUsage()
	if err != nil {
		message := err.Error()
		return ctx.JSON(http.StatusInternalServerError, codegen.ResponseInternalServerError{Message: &message})
	}
...
func composeAppsWithStoreInfo(ctx context.Context, opts composeAppsWithStoreInfoOpts) (map[string]codegen.ComposeAppWithStoreInfo, error) {
	...
	return lo.MapValues(composeApps, func(composeApp *service.ComposeApp, id string) codegen.ComposeAppWithStoreInfo {
		...
		// Translation layer: extension may be authored as x-powerlab,
		// x-web, or x-casaos — service.LookupAppExtensionMap returns
		// whichever is present.
		var isUncontrolled bool
		if extMap, _, ok := service.LookupAppExtensionMap(composeApp.Extensions); ok {
			isUncontrolled, _ = extMap[common.ComposeExtensionPropertyNameIsUncontrolled].(bool)
		}
