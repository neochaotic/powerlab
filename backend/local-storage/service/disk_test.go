...
// TestPersistentTypeConstants_NoLegacyCasaOSValue — Sprint 3 Phase 3
// rebrand. The PersistentType* constants are returned to API clients
// via /v1/storage as the `PersistedIn` field, so the value is part
// of our wire format. We're pre-v1.0 so renaming is allowed; the
// rebrand swaps the legacy "casaos" token for "powerlab" so the
// API surface is self-describing. Test pins expected values.
func TestPersistentTypeConstants_NoLegacyCasaOSValue(t *testing.T) {
	if PersistentTypePowerLab != "powerlab" {
		t.Errorf("PersistentTypePowerLab = %q, want %q", PersistentTypePowerLab, "powerlab")
	}
	if strings.Contains(PersistentTypePowerLab, "casaos") {
		t.Errorf("PersistentTypePowerLab = %q must not contain legacy 'casaos' token", PersistentTypePowerLab)
	}
	// Sanity: the other two persistence types are unchanged.
	assert.Equal(t, PersistentTypeNone, "none")
	assert.Equal(t, PersistentTypeFStab, "fstab")
}
