...
// the user sees it in the admin UI. MountPath is the URL-prefix
// the storage is exposed at; Driver names the implementation
...
// Sort embeds into StorageA — controls listing order on per-storage
// browse calls. ExtractFolder pins a folder to the top.
type Sort struct {
...
// SetStorage replaces the receiver's value with storage. Used when a
// driver re-loads its config from disk and needs to swap the live
// instance.
func (s *StorageA) SetStorage(storage StorageA) {
	*s = storage
}

// SetStatus mutates the storage's lifecycle status string (e.g.
// "ready", "loading", "error"). Surfaced to the UI on next list.
...
// DownProxyUrl. Used for storages whose direct URLs aren't browser-
// reachable (private buckets, signed URLs).
