...
if [[ -d "$ROOT/community-catalog" ]]; then
  ...
  # Emit .curated-manifest listing the bundled app slugs (ADR-0039,
  # #450). The app-management boot-time migration reads this file to
  # remove any /var/lib/powerlab/community-catalog/Apps/<dir> that is
  # NOT in the bundled set — converging upgraded boxes (v0.6.x → v0.7.x)
  # back to the curated catalog without requiring a re-install. See
  # backend/app-management/pkg/config/curated_catalog_orphan_cleanup.go.
  manifest="$STAGE/community-catalog/.curated-manifest"
  {
    echo "# Generated by scripts/package-linux.sh — DO NOT edit by hand."
    echo "# One curated app slug per line. The app-management boot migration"
    echo "# (MigrateOrphanCuratedApps) removes Apps/<dir> entries not listed"
    echo "# here. Per ADR-0039 the bundled curated set is the sole source of"
    echo "# truth for the default catalog on a clean install."
    if [[ -d "$STAGE/community-catalog/Apps" ]]; then
      # Portable across GNU and BSD find — no -printf reliance.
      for d in "$STAGE/community-catalog/Apps"/*/; do
        [[ -d "$d" ]] && basename "$d"
      done | sort
    fi
  } > "$manifest"
  log "  emitted .curated-manifest ($(grep -cv '^#\|^$' "$manifest") apps)"
fi
